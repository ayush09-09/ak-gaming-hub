...
import ReListButton from "@/components/seller/ReListButton";
...
const expiredListings = [
  { _id: "id1", nickname: "ProFF123" },
  { _id: "id2", nickname: "SniperKing" },
];
    ...
    <div className="flex">
      ...
      <div className="ml-60 p-6 w-full bg-gray-50 min-h-screen">
        ...
        <div className="bg-white p-4 rounded-lg shadow mb-6">
          <h2 className="text-lg font-semibold mb-2">⏰ Expired Listings</h2>
          {expiredListings.length === 0 ? (
            <p className="text-sm text-gray-500">No expired listings.</p>
          ) : (
            expiredListings.map((id) => (
              <div
                key={id._id}
                className="mb-4 p-4 bg-zinc-50 rounded shadow flex items-center justify-between"
              >
                <span>
                  {id.nickname}{" "}
                  <span className="text-xs text-zinc-400">(Expired)</span>
                </span>
                <ReListButton id={id._id} />
              </div>
            ))
          )}
        </div>
